...
class VideoFunding extends React.Component {
    state = {
    ...
    async componentDidMount() {
        ...
        this.props.setPath('/videoFunding');
    ...
    getVideoList = async (check) => {
        ...
        const videoData = await getAllProjects(page, 'video');
            ...
            <Layout>
                <div className="container-fluid">
                    ...
                    <div className="d-flex  flex-wrap justify-content-start  mx-xl-5 mx-md-5">
                        ...
                        {videos.length !== 0 &&
                            videos.map((video, i) => (
                                <ProjectCard video={video} key={i} type="project" />
                            ))}
                    </div>
                </div>
            </Layout>
...
VideoFunding.getInitialProps = async (ctx) => {
    const response = await getAllProjects(1, 'video');
    console.log(response);
    return {
        videos$: response.projects.docs
    };
};
export default connect(mapStateToProps, mapDispatchToProps)(VideoFunding);
