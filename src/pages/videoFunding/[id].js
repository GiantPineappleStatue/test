...
class SingleVideoFunding extends React.Component {
    constructor(props) {
        super(props);
                    ...
                    <div className="row ">
                        <div className="col-md-12 col-sm-12 col-xl-8 p-0">
                            {video && (
                                <Player
                                    ref={this.player}
                                    src={`https://artbot.mypinata.cloud/ipfs/${video.videoHash}?stream=true`}
                                    poster={`https://artbot.mypinata.cloud/ipfs/${video.imageHash}?img-width=1200`}
                                />
                            )}
...
SingleVideoFunding.getInitialProps = async (ctx) => {
    const response = await getOneProject(ctx.query.id, 'video');
    return {
